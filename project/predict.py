...
model = tf.keras.models.load_model('project/new_nn.h5')
...
def get_noutput(n_output, temp, hum, ph, rain, water):
    ...
    while count < n_output:
        new_output = get_output(temp+randint(-5, 5), hum+randint(-20, 20),
                                ph+randint(-1, 1), rain+randint(-20, 20), water)[0]
...
def get_label(output):
    output = list(output)
    crops = ['rice', 'wheat', 'Mung Bean', 'Tea', 'millet', 'maize', 'Lentil', 'Jute', 'Coffee', 'Cotton', 'Ground Nut', 'Peas', 'Rubber', 'Sugarcane', 'Tobacco', 'Kidney Beans',
             'Moth Beans', 'Coconut', 'Black gram', 'Adzuki Beans', 'Pigeon Peas', 'Chickpea', 'banana', 'grapes', 'apple', 'mango', 'muskmelon', 'orange', 'papaya', 'pomegranate', 'watermelon']
